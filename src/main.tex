...
\[ \nabla \cdot \vec{B} = 0 \iff \oint \vec{E} \cdot \diff \vec{A} = 0 \]
\[ \nabla \times \vec{B} = -\upsilon_0 \vec{J} + \mu_0 \epsilon_0 \frac{\partial \vec{E}}{\partial t} \iff \oint -\upsilon_0 I + \mu_0 \epsilon_0 \int \frac{\partial \vec{E}}{\partial t}\]
...
Resulta que la filosofía platónica propone un esquema a aplicar en distintas áreas del saber, el llamado \textit{dualismo Platónico}. Ya que
es así, exponemos el esquema general aquí y lo instanciamos posteriormente en las áreas pertinentes.

La etimología de dualismo ya sugiere el fundamento que profesa, a saber: la existencia de dos principios fundamentales que componen un
sistema. El dualismo platónico además precisa la esencia de estos dos principios, o \textit{mundos} y la relación entre ellos. Aplica el
dualismo Platónico donde hay un mundo inferior (\textit{contingente, sensible, perecedero, \dots}, en definitiva, \textit{Hericlíteo})
tiene una cierta correspondencia o subordinación (\textit{participación, imitación}) a otro superior (\textit{inmutable, inteligible, 
eterno}, en definitiva, \textit{Parménico}) que además es independiente y en cierto sentido anterior.

La aplicación del anterior método a la metafísica resulta en la llamada \textit{Teoría de las ideas}. Dicha teoría toma en el esquema
anterior \textit{lo perceptible por los sentidos} como \textit{mundo inferior}. Asimismo, el correspondiente mundo superior es el 
\textit{mundo de las ideas}. Dicho mundo, también postulado por Platón, es constituido integramente por \textit{ideas}.

% Bien como la idea perfecta, el Sol de este mundo
...
El alma, según Platón, tiene carácter divino

\subsubsection{Cuerpo}

Siendo el cuerpo el antítesis del alma, 
