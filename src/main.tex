...
Monoides ya encontramos muchos naturalmente en matemáticas (por ejemplo: $ (\N, +, 0)$, $(\Z, \times, 1)$, 
$ (\text{End}(X), \circ, 1_X) $ donde $ X $ es un conjunto \dots). Y aún así, nos interesa añadir más estructura, hasta llegar
al concepto de \textit{espacio vectorial}.

\begin{definition}[Grupos]
  Sea $ \mathcal{G} $ un conjunto. Un \textit{grupo} es un monoide para el que: 
  \[ \forall x \in \mathcal{G}, \ \exist x^{-1} \in \mathcal{G}, \ x \cdot x^{-1} = x^{-1} \cdot x = \epsilon \]
...
El ejemplo de grupo canónico es el conjunto de biiyecciones de algún conjunto a sí mismo.
...
Los números enteros con la suma y la multiplicación entera forman un anillo. El ejemplo de anillo canónico es el conjunto
de funciones sobre algún espacio.
...
Una onda se define como la alteración harmónica de alguna propiedad a lo largo del espacio o del tiempo.
